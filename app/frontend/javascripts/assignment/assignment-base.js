var assignmentBase = module.exports = {};
    // function list
    // 上传视频
    // 获取任务详细信息
    // 任务倒计时
...
assignmentBase.testerId = $('#tester-id').val();
assignmentBase.uploader = new QiniuChunkUpload();

//生成一个qrcode实例
assignmentBase.qrcode = new QRCode($('#upload-qrcode')[0], {
...
// 发送抢任务请求
assignmentBase.sendOrderAssignmentRequest = function (assignmentId, callback) {
  $.ajax({
...
// 发送订阅任务提醒请求
assignmentBase.sendSubscribeRequest = function (assignmentId, callback) {
  $.ajax({
...
// 发送取消任务提醒订阅请求
assignmentBase.sendUnsubscribeRequest = function (assignmentId, callback) {
  $.ajax({
...
// 抢任务
assignmentBase.orderAssignment = function (assignment) {
  var assignmentId = assignment.id;
  assignmentBase.sendOrderAssignmentRequest(assignmentId, function (data) {
    switch(data.code) {
      case 1:
        // 抢到
...
// 订阅提醒
assignmentBase.subscribeAssignment = function (assignment) {
  var assignmentId = assignment.id;
  assignmentBase.sendSubscribeRequest(assignmentId, function (data) {
    switch(data.code) {
      case 1:
        // 订阅成功
...
// 取消订阅
assignmentBase.unsubscribeAssignment = function (assignment) {
  var assignmentId = assignment.id;
  assignmentBase.sendUnsubscribeRequest (assignmentId, function (data) {
    switch(data.code) {
      case 1:
        // 取消订阅成功
...
// 获取assignments分页数据
assignmentBase.fetchAssignmentPaging = function (type, page, callback) {
...
assignmentBase.fetchAssignment = function (assignemntId, callback) {
...
// 加载assignments下一页
assignmentBase.loadNextPage = function (vm) {
  // 防止多次加载相同数剧
  ...
  var type = vm.type,
      page = vm.page + 1;
  vm.loading = true;
  assignmentBase.fetchAssignmentPaging(type, page, function (assignments) {
...
assignmentBase.fallLoad = function () {
  // 页面高度
...
$(window).on('scroll', assignmentBase.fallLoad);
  ...
  .done(function (data, status) {
    if(data.status === 0) {
      switch(data.code) {
        ...
        case 3:
          // 任务过期
          assignmentBase.fetchAssignment(assignmentId, function (assignment) {
...
// 获取任务详情
assignmentBase.fetchAssignmentDetail = function (testerId, assignmentId, callback) {
...
// 获取生成二维码所需token
assignmentBase.fetchQrcodeToken = function (assignmentId, callback) {
...
assignmentBase.startAssignment = function (vm, assignment, event, index) {
  // 判断任务是否过期
  if(!assignment.beginner && assignment.expired_time <= 0) {
    // 任务过期
    assignmentBase.fetchAssignment(assignment.id, function (newAssignment) {
  ...
  assignmentBase.fetchAssignmentDetail(assignmentBase.testerId, assignmentId, function (project) {
    // 任务为手机应用时生成二维码
    if(assignment.device !== 'web') {
      assignmentBase.fetchQrcodeToken(assignmentId, function (token) {
        var uploadUrl = location.origin
                      ...
                      + "&id="
                      + assignmentId;
        assignmentBase.qrcode.clear();
        assignmentBase.qrcode.makeCode(uploadUrl);
      });
    }
...
function refreshQrImage (vm, event) {
  ...
  assignmentBase.fetchQrcodeToken(vm.id, function (token) {
    var uploadUrl = location.origin
                  + "/assignments/upload?"
                  + "auth_token="
                  + token
...
// 显示上传进度
assignmentBase.showUploadProgress = function (progressPercent, $progressCircle) {
...
assignmentBase.clearUploadProgresss = function ($progressCircle) {
...
setAssignmentsMinHeight = function () {
