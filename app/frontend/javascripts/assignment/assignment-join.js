$(function () {
  ...
  function loadNextPageForIng (vm) {
    ...
    var page = vm.page + 1;
    vm.loading = true;
    assignmentBase.fetchAssignmentPaging('ing', page, function (assignments) {
    ...
    for(var i = 0, len = assignments.length; i < len; i++) {
      ...
      if(assignment.expired_time <= 0) {
        ...
        if(assignment.uploading) {
          assignmentBase.fetchAssignment(assignment.id, function (assignment) {
  ...
  function init () {
    ...
    assignmentBase.fetchAssignmentPaging('ing', 1, function (assignments) {
      if(assignments.length > 0) {
        var assignment;
        for(var i = 0, len = assignments.length; i < len; i++) {
          assignment = assignments[i];
    ...
    assignmentBase.fetchAssignmentPaging('done', 1, function (assignments) {
      if(assignments.length > 0) {
