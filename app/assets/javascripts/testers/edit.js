$(function () {
  ...
  var professionSelected = $('#profession').data('profession');
  $('#profession').citySelect({
    // 不知道为什么不能从 professionselect_data.js中获取数据
    url: profession,
    nodata: 'none',
    required: false,
    prov: professionSelected[0],
    city: professionSelected[1]
  });

  // 感情状况， 性取向, 教育程度， 收入 init
  var emotion = $('#emotion').data('emotion');
  var sexOrientation = $('#sex_orientation').data('sexOrientation');
  var education = $('#education').data('education');
  var income = $('#income').data('income');
  $('#emotion select').val(emotion);
  $('#sex_orientation select').val(sexOrientation);
  $('#education select').val(education);
  $('#income select').val(income);
